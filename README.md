...
用「不排除」各多报 1 度 —— **规则的前提当天不成立**。

我给规则时说了「4 天样本、留一验证 2/4 天变差」，但**说得不够重**。
当一个规则在一半的天数里会让结果变差，它就不该被当成操作依据。

**已停用**（`verify.py` 里 `LATE_PEAK = set()`，即全部站看「预报」）。
攒够两周后若某站在留一验证里稳定，再加回来。
...
2. **全部站看「预报」** ——「重庆武汉看不排除」那条规则**已停用**
   （2026-07-31 实测是负作用，MAE 从 0.62 恶化到 1.12，见下节）
