...
# **已上线**（2026-08-03）。模式近期偏差 —— 临近预报原来完全没有
# 「模式最近偏了多少」这个信息，只有 rise_anom_3d/7d（实测升幅相对气候的
# 异常），那是观测侧的量，与「模式系统性报低了 1.5 度」是两回事。
# 而 recent_bias 在 D+1 的岭回归里是权重第一的特征（标准化系数 +0.631）。
#
# 偏差确实可预测（855 天实测，日偏差的滞后 1 天自相关）:
#   JMA +0.587  本地GFS6h +0.547  GFS +0.487  CMA +0.480
#   ECMWF +0.411  GEM +0.381  ICON +0.245
#   分站: 北京 +0.605(R²=37%)、上海 +0.494(24%)
# 滞后 7 天仍有 +0.20~+0.41 —— **不是「一会正一会负」**，这是能订正的前提。
#
# 实测（15 个月滚动回测，基线是已上线的七成员方案）:
#    9 时  0.9159 -> 0.9035  -0.0125  P=99%  ★
#   11 时  0.7432 -> 0.7397  -0.0035  P=78%
#   13 时  0.4685 -> 0.4683  -0.0003  P=52%
#   全部   0.7092 -> 0.7038  -0.0054  P=98%  ★
#   分站: 成都 -0.0144(P=98%)、武汉 -0.0144(P=97%) —— 两个最难的站
# 9 时收益最大、13 时几乎为零，符合机制: 早时次实测少、最依赖模式。
#
# 与已否决的 DEB 区分: DEB 是**分模式**的近期偏差（借鉴 PolyWeather），
# 10/11/12 时 -0.021/+0.001/-0.005 不显著。这里是 mos.csv 里那个单一的
# recent_bias 列，且在样本翻倍（第 12 次实验）之后重测才赢。
NWP_COLS["nwp_recent_bias"] = "recent_bias"
FEATS.append("nwp_recent_bias")
...
def fit_peak_prob(rows, med, names):
    ...
    #
    # **订正一次估错**: 首次提交时写着「原始概率过度自信 12~14pt」，那是拿
    # 只训在标准窗口（约 4700 站日）上的模拟量出来的 —— 而 --peak-only 实际
    # 训在**所有有 NWP 的行**上（9-14 时约 17778），15 时无 NWP 更是全库 82535。
    # 按真实数据量建出来的表显示，原始概率本来就基本准（每一档偏差 <= 2pt）:
    #      9 时  原始 70.4~100% -> 实测 84.0%   52.9~70.4% -> 61.7%
    #     12 时  原始 78.7~100% -> 实测 88.6%   55.7~78.7% -> 67.6%
    #     15 时  原始 90.0~98.4% -> 实测 95.9%
    # 所以这张表不是在救一个坏概率，作用是:
    #   1. 让印出去的数**有据可依**，而不是碰巧对（训练量一缩水就会失真 ——
    #      4700 样本那次实测就掉到 -14pt）
    #   2. 与 p_late 一致 —— 两个方向都回填，别一个校准一个不校准
    # 15 时几乎不用校准（那时早不早见顶已由观测决定）。
...
# 查表的一个维度。**必须用本时次自己的判别器**，不能拿 9 时那个 ——
#   查表分格改善（13/14/15 合计，后半 7007 站日验证）
#     + 9 时判的概率      +0.21%   几乎为零
#     + 本时次自己判的     +3.24%
# 三种切分（前半→后半 / 后半→前半 / 奇数天→偶数天）× 三个时次 = 9/9 全为正:
#   13 时 +1.6~2.0%   14 时 +6.9~7.5%   15 时 +0.6~1.7%
#
# **一个必须记住的教训**: 首次测「9 时概率」时得到 +6.62%，是假的 —— 那次的
# 概率取自**已部署的判别器**，而它在 tr+va 全量上训过，包含用来验证的后半段，
# 概率在那半段是样本内的。换成交叉折之后只剩 +0.21%。
PEAK_PROB_CUTOFFS = {9, 10, 11, 12, 13, 14, 15}
PEAK_PROB_STORE = {}
...
# 「已见顶」硬覆盖的闸门。**2026-08-24 实测否决，默认永久关闭（0）。**
# 保留代码只为「别再试第二次」——设 PLOYGON_SETTLED_GATE=<度数> 可复现。
#
# 动机 2026-08-23: 成都当天 11 时到 30 度、12-14 时掉回 27（**晴空无降水**，
# 只是风起到 5m/s），settled 判「已见顶」把预报硬覆盖成已达 30；15-17 时风落
# 又一路回到 32。那一轮所有信号同时打满且全错 —— 不排除也是 30、可下手 94%、
# 档位建议「已定，一档足够」。而同一轮的备注里写着 `GFS 33.6`。
#
# 也就是说: 我们在用三小时的观测去否决八个模式的一致预报。
#
# 实测（10-14 时，7882 行 settled 触发）按「模式 tmax − 已达」分八档，判错率
# 单调 16.3% -> 33.8%:
#     缺口 >= 1℃   2126 行  错 28.4%  |  其余 5756 行  错 20.5%
#     缺口 >= 2℃   1116 行  错 31.7%  |  其余 6766 行  错 21.1%
#     缺口 >= 3℃    511 行  错 36.0%  |  其余 7371 行  错 21.7%
# 成都那天 14 时是 33.6 - 30 = +3.6℃，正落在最差那一格。
#
# ---- 结果: 否决。9 个配置（闸门 1/2/3℃ × 时次 12/13/14）全负 ----
#
#   闸门    12 时    13 时    14 时     **目标人群** 12/13/14
#   1℃    -1.36   -1.32   -1.02      -2.6 / -3.2 / -3.0
#   2℃    -0.86   -0.93   -0.67      -2.4 / -3.7 / -3.5
#   3℃    -0.48   -0.41   -0.45      -2.3 / -3.0 / -4.5
#
# 三条判据全不过: 总体 9/9 负、双向验证 18/18 负、**目标人群最差**
# （闸门该起作用的那批行反而亏最多）。MAE 也全部变差。
#
# 总体损失随闸门升高而缩小纯粹是因为动的行变少 —— 逐行看，**每个被改动的
# 行平均都在亏**，且闸门越高改对率越低（12 时 20.3% / 17.3% / 13.2%）。
# 所以不存在能赢的门槛，更温和的变体（比如只压缩一半升幅）也不会赢。
#
# **这里的方法论错误值得记住**: 「settled 在缺口大的行上更容易错」
# （36% vs 21.7%）**不等于**「那批行不该覆盖」。那批行上不覆盖的替代方案
# —— 相信模式给的大升幅 —— 错得更离谱（只有 13.2% 改对）。settled 虽然
# 错 36%，另外 64% 里它是对的，而模型在那些日子把升幅报得离谱高。
# **条件错误率的提升不能证明反事实更好，必须把反事实真跑出来。**
#
# **同一批数据上先毙掉的两条**（先测后提，别再重复）:
#   晴空降温 vs 对流降温   方向反了且只有 1.2 倍（晴空无风变 24.5% 最差、
#                          晴空+起风 19.9% 最低）
#   拿 p_late 做仲裁       AUC 0.491 —— settled 与见顶判别器读同一批下降信号，
#                          高度相关、很少分歧，成都那轮是**一起错的**
SETTLED_GATE = float(os.environ.get("PLOYGON_SETTLED_GATE") or 0.0)


def settled_blocked(f):
    """模式说该比已达高这么多时，不做硬覆盖。闸门为 0 则永不阻断。"""
    if SETTLED_GATE <= 0:
        return False
    g = f.get("nwp_minus_sofar")
    return g is not None and g >= SETTLED_GATE
