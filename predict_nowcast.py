...
# **逐时次门槛**（2026-08-25 加）。判别力随时次单调上升（站内 AUC 9 时
# 0.707 -> 15 时 0.780），一个门槛通吃就是浪费: 晚时次信息多，门槛可以
# 放松而精度不掉，召回却能翻倍。
#
# 定法: **按标注率选**，不按概率门槛 —— 换目标后概率分布整体右移，固定
# 门槛不可比。首次尝试「精度>=45% 下召回最大」在新目标上过拟合到分位噪声
# （选出 9 时 0.70 / 12 时 0.40 / 14 时 0.60 这种无规律的门槛，9 时每天只标
# 0.01 个 = 三个月一次），已弃用。
#
# 现在统一取**标注率 5%**（每天约 0.4 个站），样本外双向:
#     时次   精度 半A→半B     召回 半A→半B    每天   对应概率门槛
#      9时  36.7% → 36.0%   16.3% → 16.4%   0.37      0.38
#     12时  39.3% → 38.1%   17.5% → 17.1%   0.40      0.39
#     15时  42.8% → 43.1%   19.0% → 19.4%   0.40      0.41
# 曲线光滑、两半段差 <=2pt。想更准就往 2~3% 走（精度 39~49%、召回 7~12%），
# 想更全就往 8~12% 走（精度 31~40%、召回 24~39%）。
#
# **精度看起来比换目标前低**（旧目标 5% 下是 45~49%），那是分母变了 ——
# 旧的 45~49% 里混着「见顶确实晚、但整数没变」的假阳性。按同一把尺子
# （标出来的日子里整数真变的比例）新目标全面更优，见 fit_peak_prob2 的记录。
# **2026-08-26 从标注率 5% 放到 8%** —— 用户要「全并且准」，而这两件在
# 站内 AUC 0.68~0.75 下不可兼得，只能在曲线上挪。8% 比 5% 多标六成，
# 精度只掉 2~3pt:
#     标注率   精度        召回        每天标
#      2%    39~49%     7~12%      0.16
#      5%    36~43%    16~19%      0.40   (原)
#      8%    34~40%    24~29%      0.64   (现)
#     12%    31~37%    33~39%      0.96
# 实例: 2026-08-26 重庆 15 时给 40%，5% 的门槛是 0.41 —— 差 1 个百分点没标，
# 而它当天确实在 16 时把整数从 37 改成 38。8% 的门槛能标出来。
PEAK_WARN_BY_CUT = {9: 0.32, 10: 0.32, 11: 0.32, 12: 0.32, 13: 0.34,
                    14: 0.34, 15: 0.34}
PEAK_WARN_TH = 0.40          # 兜底: 时次不在表里时用它
...
def main() -> int:
    ...
    def _bucket_advice(cut, rise, shown, p_late=None):
        ...
        # 一档已经够好就买一档；否则两档；两档还不到 70% 就提三档。
        #
        # **门槛 2026-08-27 从 0.85 提到 0.90。** 0.85 是拿建表时的**点估计**
        # 挑格子，而越过门槛的格里有一部分是靠向上的噪声越过去的 —— 典型的
        # 胜者诅咒，样本越小回归越狠。拿 2025-05~2026-08 的回测逐格核对
        # （25 格）: 24 格与表内值差在 ±0.043 以内，**只有触发「买一档」的
        # 那几格系统性塌陷，且塌陷幅度精确跟着建表样本量走**:
        #     格     建表 n   表内一档   回测实测      差
        #     9|0      159    0.880    0.741   -0.140
        #    10|0      174    0.885    0.730   -0.155
        #    11|0      266    0.868    0.734   -0.134
        #    14|0     2090    0.884    0.803   -0.081
        #    15|0     3694    0.928    0.928   -0.000   <- 唯一 >=0.90 的，分毫不差
        # 14|0 那 8pt 在 n=2090 下是 11 个标准误，不是抽样噪声；它在四个时段
        # （2025 夏/秋冬、2026 冬春/夏）分别是 76.1/83.8/81.7/78.7%，从没接近
        # 88.4% —— 整个窗口上那个值都不成立。而 0.90 以上那格分毫不差。
        #
        # 双向验证（28516 行，时次 10~15，对半切）:
        #     半段   全样本覆盖         平均买档        受影响行覆盖
        #     前半  86.09% -> 89.30%  2.158 -> 2.319  76.1% -> 96.0%（2231 行）
        #     后半  85.80% -> 88.72%  2.190 -> 2.342  76.4% -> 95.7%（2088 行）
        # 两半段一致，+2.9~3.2pt 覆盖，代价平均多买 0.16 档。
        #
        # 试过没赢:「闸门改用 Wilson 95% 下界」—— 能修 9/10/11 时那三格
        # （n 小、下界掉到 0.82），但修不了 14|0（n=2090，下界 0.869 仍过线，
        # 而实测 0.803）。那格的问题不是抽样噪声。
        # 试过等价:「`已见顶` 钳过的行一律抬两档」—— 双向 89.13%/88.52%，
        # 比门槛法低 0.17~0.20pt，且够不到 9 时，还要把 settled 状态传进来。
        if one >= 0.90:
    ...
    def _peak_prob2(stn, hrs):
        ...
        if not _PEAK2 or stn not in _PEAK2:
            return None
        g = _PEAK2[stn]
        # 生产端把已经取到的模式数据一并喂进去 —— fetch_nwp / fetch_m2 的
        # 返回结构与 mos.csv 同源，所以两端用的是同一套键。取不到就是 nan，
        # 判别器照常出数（训练时 2024 年前也全是 nan）。
        x = N.peak2_feats(hrs, cutoff, tgt.isoformat(),
                          nwp.get(stn), [mm.get(stn) for mm in m2])
        if x is None:
            return None
    ...
    for stn in stations:
        ...
        #
        # **2026-08-25 把后两处的 A/B 做了，结论是不换。** 判别力更强不等于
        # 在每个用途上都更好 —— 两处的目标与 v2 的调参方向相反:
        #
        #   档位建议要的是**召回**（宁可多买一档也别漏，代价极低）。v2 按精度
        #   调的门槛让抬档行数从 690~723 掉到 28~42，覆盖增益从 +0.65/+1.90pt
        #   缩到 +0.05/+0.12pt。放低门槛追召回后成本对齐再比: 没有一个门槛在
        #   两个方向同时占优（后半方向 0.05 赢 +0.06 覆盖/-0.009 成本，前半
        #   方向要 0.10 才打平，而 0.05 在那边是纯多花钱）。双向不一致，不改。
        #
        #   「不排除」下限要的是覆盖率（它是 90 分位，覆盖是契约）。v2 的白抬率
        #   确实更低（80~84% vs 87%），但抬的行数少太多（后半 151 vs 571），
        #   **全样本覆盖反而掉 0.8pt** —— 违约了，不改。
        #
        # 记这一条是为了别再试第二次: **判别力提升只在「按精度筛选」的用途上
        # 兑现，在「宽而浅地兜底」的用途上反而是负担。**
    ...
    # ── 今日研判。**只做汇总，不引入新判断。**
    #
    # 起因: 用户要求「早上就把今天的形势点出来，别晚上回顾才说」。合理 ——
    # 但我先量了一遍**日尺度**的上午条件（云量/风速/湿度/有雨/升温幅度）
    # 能不能预告当天准不准，481 天实测: 跨度只有 4~9pt，且**控制住剩余升幅
    # 之后云量的残余信息方向不一致**（<0.25℃ 档多云更准 +6.2pt、>=2℃ 档
    # 少云更准 +3.2pt），是噪声。
    #
    # 也就是说: **「预计再升」和「预期命中」已经把日尺度可预报性讲完了**，
    # 没有独立于它们的日尺度信息可讲。所以这里不造新判断，只把已经算出来、
    # 但散在三张表里要自己拼的东西汇到一处:
    #   平均剩余升幅（决定今天整体有多难）、各站已定/待定、⚠ 名单、
    #   模式离散最大的站。
    if rows_out:
        _r = [r[3] for r in rows_out]
        _avg = sum(_r) / len(_r)
        _done = [r[0] for r in rows_out if r[3] < 0.25]
        _open = [r[0] for r in rows_out if r[3] >= 2.0]
        _wl = [x[0] for x in _peaks
               if (x[4] if x[4] is not None else x[3]) is not None
               and (x[4] if x[4] is not None else x[3])
               >= PEAK_WARN_BY_CUT.get(cutoff, PEAK_WARN_TH)]
        print(f"\n── 今日研判（{cutoff} 时）")
        print(f"  全站平均还剩 {_avg:.2f}℃ 没发生。"
              f"**这是今天整体难度的唯一可靠指标** —— 实测按剩余升幅分档，"
              f"<0.25℃ 的站日完全命中 71~77%、0.25~1℃ 掉到 44~49%、"
              f">=2℃ 只有 39~42%。上午云量/风速/湿度在控制住它之后没有增量。")
        if _done:
            print(f"  已基本定死（剩余 <0.25℃，历史命中 71~77%）: "
                  f"{'、'.join(N.NAMES.get(x, x) for x in _done)}")
        if _open:
            print(f"  变数最大（剩余 >=2℃，历史命中 39~42%）: "
                  f"{'、'.join(N.NAMES.get(x, x) for x in _open)}")
        if _wl:
            print(f"  ⚠ 16 时后还在涨、整数可能被改: "
                  f"{'、'.join(N.NAMES.get(x, x) for x in _wl)}"
                  f"  —— 这一列精度 34~40%、召回 24~29%，"
                  f"**标了值得当回事，没标不等于安全**")
        elif _peaks:
            print(f"  ⚠ 今天没有站达到「整数会被改」的门槛。"
                  f"召回只有 24~29%，**不标不等于不会发生**。")
    ...
    if _peaks:
        print(f"\n── 16 点后还在涨、会改掉整数的风险（**不改上面的预报值**）")
        _PW = (16, 9, 11, 10, 10, 9, 7)
        print("  " + F.L("站点", _PW[0])
              + "".join(F.R(t, x) for t, x in
                        zip(("早<13时", "正常13-15", "晚>=16时", "会改整数",
                             "该站常年", "倍数"), _PW[1:]))
              + "   提示")
        ...
        for _s, _e, _n, _l, _c, _b in sorted(_peaks, key=lambda x: -x[3]):
            ...
            _wth = PEAK_WARN_BY_CUT.get(cutoff, PEAK_WARN_TH)
            # 「基本能早早定下来」**2026-08-27 删掉**。
            #
            # 它印的是 P(首达 < 13 时) —— 一个关于**时刻**的判断，可那句话
            # 读起来是「这个数定了，可以下手」，是关于**值**的。两者在早时次
            # 根本不是一回事: 9 点温度才刚起步，就算确知 11 点见顶，你对那个
            # 值仍然一无所知。实测（2026-08-22~26，51 次标注）「标了之后整数
            # 还改了没有」:
            #      9时 0/6      10时 2/6     11时 1/4     12时 3/6
            #     13时 6/11    14时 7/11    15时 7/7      合计 26/51 = 51%
            # **9 时标的六次，六次整数后来都改了。** 而同一张主表上的
            # 「可下手 ✓」答的正是这个问题，实测 59/60 = 98%（回测承诺
            # 98.9%），头次出 ✓ 平均 16.3 时。这条提示在任何时次都不可替代
            # 性为零 —— 15 时它 100% 对，但那时 ✓ 早就在了。
            #
            # 与 2026-08-26 给 ⚠ 换目标（首达>=16 -> 整数会改）是同一类错，
            # 只是「早早」当时没跟着换。左边「早<13时」那一列仍然照印，
            # 它本来就标着「只作参考」。
            _hint = ("⚠ 16 点后还在涨，整数可能改"
                     if (_use is not None and _use >= _wth) else "")
            ...
            print("  " + F.L(f"{_s} {N.NAMES.get(_s, '')}", _PW[0])
                  ...
                  + f"   {_hint}")
        print(f"  **「会改整数」这一列（以及 ⚠）判的是「16 时之后还在涨、"
              f"以至于报出去的整数会变掉」，不是「几点见顶」。** 2026-08-26 换的目标: "
              f"首达>=16 的日子里只有 82.7% 真的改变了整数，其余是「见顶确实晚、"
              f"但只多涨不到半度」—— 对整数盘口没有影响，不该报警。")
        print(f"  左边「早<13时/正常13-15/晚>=16时」三列仍是旧判别器的见顶时刻分档，"
              f"只作参考。分档: 早=<13 时 / 正常=13-15 时 / **晚=>=16 时**。"
        ...
        print(f"  全站基础率 {_BASE:.0%}（成都 29% / 重庆 23% / 武汉 19% / "
              ...
              f"—— **不是「极有可能」，是「风险高 3 倍」**。")
        _wth = PEAK_WARN_BY_CUT.get(cutoff, PEAK_WARN_TH)
        print(f"  {cutoff} 时的阈值 {_wth:.0%}（逐时次定，2026-08-25 起；判别器同时"
              f"换成分站/纯观测版）: 样本外精度 46~49%，两个半段差 <=3pt。")
        print(f"  代价是召回低 —— 9/10 时只有 10~13%（每天标 0.25 个）、"
              f"15 时 30~32%（0.70 个）。**早时次注定标得少**，那是上午信息量的"
              f"上限，不是调参问题。**标了值得当回事，没标不等于安全。**")
        print(f"  「该站常年」是该站自己的历史晚见顶率，「倍数」= 校准后 / 常年 ——"
              f" 成都常年 24%、上海 2%，同样印 30% 对两个站的含义完全不同。")
        print(f"  「早<13时」这一列只作参考，**不再据它印任何提示**"
              f"（原「基本能早早定下来」2026-08-27 删除）。它判的是见顶**时刻**，"
              f"不是「这个数定了没有」—— 实测标了之后整数仍会改的比例，"
              f"9 时 6/6、12 时 3/6。要判「能不能下手」看主表的「可下手 ✓」，"
              f"那一列实测 59/60。**⚠ 不改预报值。**")
    ...
    if args.p90:
        ...
        # **2026-09-02 改口，2026-09-04 再更正。** 原文印的是「实测覆盖 94%
        # （比目标 90% 还保守）」—— 那是**回测**数字。
        #
        # 09-02 那次量到 08-22~09-02 合计 86%、七个时次全部低于 90%，据此写成
        # 「系统性未达契约」。**那个结论下早了** —— 12 天窗口里 10 天是同一段
        # 热浪，被它主导了。分开看:
        #     08-22~31（热浪）  631 行  85%  CI [82%,87%]   未达契约
        #     09-01~04          210 行  94%  CI [90%,96%]   达标
        # 「预期命中」同样: 8 月下旬高估 6pt，9 月反而保守 2pt。
        #
        # 物理上讲得通: 入秋后当天定型更早（15 时已定型 91% vs 8 月 85%），
        # 剩余升幅小，P90 容易满足。**所以这是热季问题，不是常年问题。**
        # 教训记在这里: 拿一个被单一天气时段主导的短窗口下「系统性」结论，
        # 正是本仓库反复栽的那个坑，这次是检查者自己踩的。
        #
        # 不是那两条钳制造成的: 被自洽钳制压过的行覆盖 93%（86 行），没被压过的
        # 才是 85%（615 行，CI [82%,87%]）—— 是 q90 原始输出在生产上就偏低。
        # 已知成因之一是 load_hourly 的整点分桶泄漏（训练看得到 :15 后的报文，
        # 生产看不到，「已达」被抬高 0.128℃），但漏掉的 80/99 差整整 1 度，
        # 泄漏解释不了全部。**根因未定，所以只改口不改模型** —— 在 12 天数据上
        # 给 p90 加偏移是拿噪声调参。
        print("  它是升幅的条件 P90，目标覆盖 90%。**热季会掉到契约以下** ——")
        print("  实测: 2026-08-22~31（热浪时段）85%、未达契约；9 月 94%、达标。"
              "回测全窗口 94.3%。")
        print("  作用是「极端日心里有数」，**别把它当上界**: 没盖住时九成只低 1 度。")
