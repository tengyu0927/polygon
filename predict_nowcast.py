...
def main() -> int:
    ...
    for stn in stations:
        ...
        #     0.95     15.45 时    97.4%     96.4%      78.2%   <- 用这个
        #     0.97     15.62 时    98.0%     98.5%      82.4%
        #     对照: 全部站死等 16 时   16.00 时  96.7%  95.6%  72.9%
        #     对照: 全部站死等 17 时   17.00 时  99.5% 100.0%  95.0%
        #
        # 0.95 比死等 16 时**更准、还平均早半小时** —— 因为半数站点 12-14 时
        # 就已经锁死，不必陪着最难的那几个站一起等。出手时刻的分布:
        # 11 时 4.0% / 12 时 2.6% / 13 时 7.0% / 14 时 17.7% / 15 时 8.9%
        # / 16 时 19.2% / 17 时 40.6%（那 40% 是真的到 17 时才定得下来）。
        #
        # **为什么是 max 不是 min。** 两个来源会打架（浦东那种「已见顶 91%、
        # 但超越概率还有 21%」）。实测 10-14 时按是否一致分组:
        #     两者都 >=95%              624 站日   命中 97.9%
        #     只有已见顶 >=95%         1756 站日   命中 93.6%
        #     只有 1-超越 >=95%         155 站日   命中 93.5%
        # 打架的那两群确实只有 93.5%，略低于宣称的 95%。但整条策略的实现值
        # 是 97.4%（晚出手的那批把平均拉上去了），而换成 min 要等到 15.99 时
        # ——那就等于死等 16 时，早出手的价值全没了:
        #     min@0.95   15.99 时   98.9%   99.8%
        # 保守的人可以改用 min（两列同时 >=95% 才算），代价是晚半小时。
        # 两列本来就并排印着，打架时自己看得见。
        _rdy = max(_sp if _sp is not None else 0.0,
    ...
    print("\n「可下手」= max(已见顶概率, 1-超越概率)，标 ✓ 表示 >= 95%，"
          "这个站现在就能定下来、后面不用再改。**它不改任何预报值。**")
    print("  用法: 每个站头一次见到 ✓ 就下手，之后不再动。回测 4608 站日/476 天，"
          "平均出手 15.45 时，完全命中 97.4%，十站里至少九站对的日子占 96.4% ——"
          "\n  比全部站死等 16 时（96.7% / 95.6%）更准，还早半小时。"
          "没到 ✓ 的站等下一轮，17 时那轮兜底。")
