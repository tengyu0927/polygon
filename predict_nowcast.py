...
def main() -> int:
    ...
    if args.p90:
        ...
        # **2026-09-02 改口。** 原文印的是「实测覆盖 94%（比目标 90% 还保守）」
        # —— 那是**回测**数字，生产达不到。线上实测（08-22~09-02，701 行，
        # 每时次内各站日独立）:
        #     9时 85% / 10时 81% / 11时 88% / 12时 84% / 13时 86% / 14时 88% / 15时 88%
        # **七个时次全部低于 90% 契约**，符号检验 p=0.008，合计 86%。
        #
        ...
        # 给 p90 加偏移是拿噪声调参。
        print("  它是升幅的条件 P90，目标覆盖 90%。**线上实测只有 86%，未达契约**"
              "（回测 94% 是偏乐观的，见代码注释）——")
        print("  作用仍是「极端日心里有数」，但**别把它当上界**: 每 7 次里约有 1 次"
              "实际比它还高，且九成只高 1 度。")
