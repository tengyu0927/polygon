...
# 放松而精度不掉，召回却能翻倍。
#
# 定法: 在「两个半段精度都 >= 45%」的前提下取召回最大的门槛（取两半段里
# 差的那个当判据，保守）。样本外，按年奇偶双向:
#     时次   门槛   标注率    精度 半A→半B     召回 半A→半B   每天标几个
#      9时   0.50   3.3%    48.3% → 46.7%   13.5% → 10.1%    0.25
#     10时   0.50   3.3%    49.5% → 48.2%   13.3% → 10.7%    0.25
#     11时   0.45   5.2%    48.4% → 46.7%   19.5% → 17.1%    0.41
#     12时   0.45   5.4%    48.4% → 46.3%   20.4% → 17.3%    0.43
#     13时   0.45   5.5%    49.4% → 47.7%   21.3% → 18.2%    0.44
#     14时   0.40   8.1%    46.3% → 46.5%   29.3% → 26.5%    0.65
#     15时   0.40   8.7%    48.4% → 47.4%   32.3% → 29.6%    0.70
#
# **2026-08-26 随判别器加 NWP 廓线一起重定。** 判别力涨了，同样精度下门槛
# 可以放低、召回跟着涨 —— 增量集中在早时次，正是原来最弱的一环:
#     时次   门槛   标注率    精度 半A→半B     召回 半A→半B   每天   (v2 时召回)
#      9时   0.45   4.8%    45.3% → 45.6%   18.2% → 14.6%   0.36   (13.5→10.1)
#     10时   0.45   4.8%    45.1% → 45.4%   17.7% → 14.7%   0.37   (13.3→10.7)
#     11时   0.45   5.3%    48.3% → 49.3%   20.1% → 17.8%   0.42   (19.5→17.1)
#     12时   0.45   5.3%    48.1% → 47.9%   20.5% → 17.1%   0.42   (20.4→17.3)
#     13时   0.40   7.6%    45.7% → 45.6%   27.4% → 24.1%   0.61   (21.3→18.2)
#     14时   0.40   8.1%    46.3% → 47.1%   29.9% → 26.3%   0.65   (29.3→26.5)
#     15时   0.40   8.7%    47.5% → 48.4%   32.0% → 29.7%   0.70   (32.3→29.6)
# 9/10 时召回从 10~14% 提到 15~18%（**涨四成**），13 时从 18~21% 提到 24~27%，
# 精度全部保持 45~49%、两半段差 <=1.9pt。
PEAK_WARN_BY_CUT = {9: 0.45, 10: 0.45, 11: 0.45, 12: 0.45, 13: 0.40,
                    14: 0.40, 15: 0.40}
...
def main() -> int:
    ...
    if _peaks:
        ...
        for _s, _e, _n, _l, _c, _b in sorted(_peaks, key=lambda x: -x[3]):
            ...
            _hint = ("⚠ 16 点后见顶风险偏高" if (_use is not None and _use >= _wth)
                     else ("基本能早早定下来"
