...
# 这两站看「不排除」，其余 6 站看「预报」。
#
# 依据（2026-07-26~07-31，6 天 312 条）: 全看预报 MAE 0.740，本规则 0.593。
# 按天配对 bootstrap 95%CI [-0.250, -0.015]，P(规则更好)=98%。逐日 4 胜 1 平 1 负。
# 留一天验证中武汉、重庆 6/6 折都被选中；北京 4/6、广州 2/6、深圳 2/6 属折间闪烁的
# 噪声，**不要**因为某天它们好看就加进来（07-28 加了广州深圳就把当天从赢变成输）。
#
# 07-31 是那 1 个负日: 重庆 14 时见顶、武汉预报本就正确，各多报 1 度。
# 曾据此单日停用本规则 —— 那是错的，用 1 天推翻 5 天。改动本集合的唯一依据是
# 下面这条预先约定的判据，不是最近一天的表现:
#
#   每周五重跑 `python3 verify.py --report`，仅当某站累计 P(不排除更好) 连续两周
#   落在 90% 以下才移出；新站加入同样要求 P≥95% 且留一天验证全折入选。
LATE_PEAK = {"ZHHH", "ZUCK"}
