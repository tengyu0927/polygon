...
# **空集 = 所有站都看「预报」。这是 2026-08-02 用 15 个月回测定下的，
# 不是又一次按几天表现摇摆。**
#
# 这条规则曾经存在（武汉、重庆看「不排除」），依据是 2026-07-26~08-02
# 的 8 天前瞻样本: 全看预报 0.736 / 规则 0.675 / P=98%（后降到 77%）。
# **8 天太少了，而且那正好是一波持续高温、模式集体报低的天气型 ——
# 在那种天气型里「报高一点」当然赢。**
#
# 用 15 个月的滚动回测（11064 条，1458 天/站，是那 8 天的 180 倍）重算:
#
#   最终值 = 预报 + w×(不排除 − 预报)
#     w=0.0 纯预报      MAE 0.7299   ±1℃ 86.3%
#     w=0.5             0.9375        79.3%
#     w=1.0 纯不排除     1.3247        63.7%
#
#   分站最优 w: 武汉 0.00、重庆 0.15，其余大多 0.00 —— **没有一个接近 1.0**。
#   武汉用不排除 MAE 1.257 vs 用预报 0.754，误差几乎翻倍。
#
# 各目标人群（晚见顶日 / 大升幅日 / 还会升的日子 / 已见顶日 / 只看武汉重庆）
# **全部是「用预报」更好**。穷举 8 站 × 3 时次 × 3 种见顶类型 = 72 个子集，
# 只有 6 个（8%）用不排除更好 —— 多重比较的产物。
#
# **教训**: 小样本上「显著」的规则，在大样本上可以是完全错的。
# 前瞻样本珍贵，但 8 天不足以定规则；有回测能覆盖的问题就该用回测定。
#
# 「不排除」不是备选预报值，它是 94% 覆盖率的上界，作用是「极端日别被打懵」，
# **不该拿来当每天要报的那个数**。
LATE_PEAK: set[str] = set()
