...
# **已停用**。2026-07-31 实测这条规则是负作用: 那天重庆 14 时就见顶、
# 武汉虽 16 时见顶但预报本就正确，用「不排除」各多报 1 度，
# 13 时 MAE 从 0.62（全看预报）恶化到 1.12。
# 教训: 4 天样本选出来的规则，留一验证已显示 2/4 天变差，本就不该当操作依据。
# 这里保留空集合 = 全部站都看「预报」。攒够两周后若某站稳定再加回。
LATE_PEAK = set()
