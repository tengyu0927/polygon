...

{
    ...
    # **必须显式传 --date。** 记账器不给日期时用 `datetime.now()`，而本脚本
    # 23:59 启动、跑到这里已过十几分钟 —— 早过午夜了，于是它去记「明天」，
    # 明天没数据就每晚静默跳过。2026-08-31 查出来时，记账器上线以来从 cron
    # 里一条都没记成过（库里的数据全是手动回填的），而日志里只有一行
    # 「峰值时段还没走完，本次不记」，看着像正常拦截。
    # TODAY 在脚本第 49 行、启动时刻取，是对的那一天。
    python3 peak_hint_track.py --log --date "$TODAY" || true
} 2>&1 | tee -a "$LOG"
